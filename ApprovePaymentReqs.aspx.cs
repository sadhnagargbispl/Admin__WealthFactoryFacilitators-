    ...
    private void AprvAction(string AprvType, string ApprvStatus)
    {
        try
        {
            ...
            foreach (GridViewRow Gvr in GvData.Rows)
            {
                ...
                if (Chk.Checked == true && Chk.Enabled == true)
                {
                    if (AprvType == "Y")
                    {
                        ...
                        //   "'0','" + lbl.Text + "'," + LblAmount.Text + ",'Amount Added by Payment  Req.No.:" + lblReqno.Text + ".','Req/" + lblReqno.Text + "','W','C',Convert(Varchar,Getdate(),112),'" + Session["CurrentSessn"] + "','A',1 FROM TrnVoucher;";
                    }
                    else
                    {
            ...
            if (a != 0 && Cnt > 0)
            {
                ...
                //scrname = "<SCRIPT language='javascript'>alert('" + Cnt + " Requests " + MsgTxt + " Successfully.');location.replace('ApprovePaymentReqs.aspx');" + "</SCRIPT>";
                //ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "alert", "alert('" + Cnt + " Requests " + MsgTxt + " Successfully.');", true);
                //return;
                lblMsg.Text = Cnt + " Requests " + MsgTxt + " Successfully.";
                lblMsg.ForeColor = System.Drawing.Color.Green;
                lblMsg.Visible = true;

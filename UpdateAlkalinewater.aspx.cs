...
public partial class UpdateAlkalinewater : System.Web.UI.Page
{
    string GroupIdQS;
    ...
    private void BindData()
    {
        ...
        try
        {
            string sql = objDal.IsoStart + "Select * From " + objDal.DBName + "..AlkalineWaterJug  Where id = '" + KitIdQS + "' " + objDal.IsoEnd;
            Dt = new DataTable();
            ...
            if (Dt.Rows.Count > 0)
            {
                ...
                if (txtcourierdate.Text == "")
                {
                    ...
                    txtcourierdate.Text = Convert.ToDateTime(Dt.Rows[0]["Approvedate"]).ToString("yyyy-MM-dd");
    ...
    protected void BtnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            Sql = "Update AlkalineWaterJug set CardNo='"+ txtcardno.Text + "',CourierName='"+ txtcouriername.Text + "',DocketNo='"+ txtdocketno.Text +"',userstatus='" + rdblist.SelectedValue + "',approvedate ='"+txtcourierdate.Text + "',RectimeStamp=getdate(),ApproveRemark = '" + txtRemarks.Text + "'";
            ...
            if (updateEffect > 0)
            {
                if (updateEffect > 0)
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Key", "alert('Successfully Updated.!');location.replace('AlkalineWaterReport.aspx');", true);
                }
            }
            else
            {
    ...
    {
        string sql = objDal.IsoStart + "Select * From " + objDal.DBName + "..AlkalineWaterJug  Where cardno = '" + txtcardno.Text + "' " + objDal.IsoEnd;
        Dt = new DataTable();
        Dt = SqlHelper.ExecuteDataset(constr1, CommandType.Text, sql).Tables[0];
        if (Dt.Rows.Count > 0)
        {

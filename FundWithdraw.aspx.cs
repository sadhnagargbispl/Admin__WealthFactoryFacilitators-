    ...
    private void FillDetail()
    {
        try
        {
            ...
            if (dtData.Rows.Count > 0)
            {
                ...
                //lbltdscharge.Text = "TDS charge: " + dtData.Compute("Sum(TdsAmount)", "");
                //lbladmincharge.Text = "Service charge: " + dtData.Compute("Sum(AdminCharge)", "");
        ...
        try
        {
            ...
            foreach (GridViewRow Gvr in GvData.Rows)
            {
                ...
                if (Chk.Checked)
                {
                    ...
                    string lblTdsAmount = ((Label)Gvr.FindControl("lblTdsAmount")).Text;
                    ...
                    string RemarkTds = "Request Rejected: Fund Debited Against TDS with req. no. " + Id;
                    string RemarkBC = "Request Rejected: Fund Debited Against Bank Admin Charge with req. no. " + Id;
                    ...
                    Str_TrnFun.Append(Convert.ToDecimal(lblNetAmount) + ",'" + Remark + "','" + refNo + "','E','C',CONVERT(VARCHAR,GETDATE(),112),");
                    Str_TrnFun.Append(sessQuery + " FROM TrnVoucher;");
                    // 3. Append Second INSERT (Admin Charge reversal)
                    Str_TrnFun.Append("INSERT INTO TrnVoucher(VoucherNo,VoucherDate,DrTo,CrTo,Amount,Narration,RefNo,AcType,VTYpe,SessID,WSessID) ");
                    Str_TrnFun.Append("SELECT ISNULL(MAX(VoucherNo)+1,1001),'" + voucherDate + "','0','" + FormNo + "',");
                    Str_TrnFun.Append(Convert.ToDecimal(lblAdminCharge) + ",'" + RemarkBC + "','" + refNo + "','E','C',CONVERT(VARCHAR,GETDATE(),112),");
                    Str_TrnFun.Append(sessQuery + " FROM TrnVoucher;");
                    // 4. Append Third INSERT (TDS reversal)
                    Str_TrnFun.Append("INSERT INTO TrnVoucher(VoucherNo,VoucherDate,DrTo,CrTo,Amount,Narration,RefNo,AcType,VTYpe,SessID,WSessID) ");
                    Str_TrnFun.Append("SELECT ISNULL(MAX(VoucherNo)+1,1001),'" + voucherDate + "','0','" + FormNo + "',");
                    Str_TrnFun.Append(Convert.ToDecimal(lblTdsAmount) + ",'" + RemarkTds + "','" + refNo + "','E','C',CONVERT(VARCHAR,GETDATE(),112),");
                    Str_TrnFun.Append(sessQuery + " FROM TrnVoucher;");
                    // Execute the entire query block at once
                    updateeffect = SqlHelper.ExecuteNonQuery(constr, CommandType.Text, Str_TrnFun.ToString());
                    //StringBuilder Str_TrnFun = new StringBuilder();
                    //string Remark = "";
                    //string RemarkTds = "";
                    //string RemarkBC = "";
                    //RemarkTds = "Request Rejected: Fund Debited Against TDS with req. no. " + Id;
                    //RemarkBC = "Request Rejected: Fund Debited Against Bank Admin Charge with req. no. " + Id;
                    //Remark = "Request Rejected: Fund Debited Against Bank Withdrawal with req. no. " + Id;
                    //string refNo = "REJECTReq/" + Id;
                    ...
                    //// First INSERT
                    //Str_TrnFun.Append("INSERT INTO TrnVoucher(VoucherNo,VoucherDate,DrTo,CrTo,Amount,Narration,RefNo,AcType,VTYpe,SessID,WSessID) ");
                    //Str_TrnFun.Append("SELECT ISNULL(Max(VoucherNo)+1,1001),'" + DateTime.Now.ToString("dd-MMM-yyyy") + "','0','" + FormNo + "',");
                    //Str_TrnFun.Append(Convert.ToDecimal(lblNetAmount) + ",'" + Remark + "','" + refNo + "','E','C',CONVERT(VARCHAR,GETDATE(),112),");
                    //Str_TrnFun.Append("(Select Max(SessID) from M_SessnMaster) FROM TrnVoucher;");
                    //// Second INSERT
                    //Str_TrnFun.Append("INSERT INTO TrnVoucher(VoucherNo,VoucherDate,DrTo,CrTo,Amount,Narration,RefNo,AcType,VTYpe,SessID,WSessID) ");
                    //Str_TrnFun.Append("SELECT ISNULL(Max(VoucherNo)+1,1001),'" + DateTime.Now.ToString("dd-MMM-yyyy") + "','0','" + FormNo + "',");
                    //Str_TrnFun.Append(Convert.ToDecimal(lblAdminCharge) + ",'" + RemarkBC + "','" + refNo + "','E','C',CONVERT(VARCHAR,GETDATE(),112),");
